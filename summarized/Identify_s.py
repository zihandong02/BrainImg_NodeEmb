            ...
            for iteration in range(num_iterations):
                ...
                if (iteration+1) % iter_N == 0:
                    ...
                    with torch.no_grad():
                        ...
                        _, top100_indices = torch.topk(l2_norms, 100)
...
# Separate the data based on c_0
data_0 = l2_norms[c_0 == 0]
data_1 = l2_norms[c_0 == 1]

# Calculate k and the k-th largest value
k = np.sum(c_0 == 1)
kth_largest = np.sort(l2_norms)[-k]  # Sort and get the k-th largest value

# Create the violin plot
plt.violinplot([data_0, data_1], positions=[0, 1], showmeans=True)

# Add the k-th largest value as a horizontal line
plt.axhline(y=kth_largest, color='red', linestyle='--', label=f'{k}-th Largest Value')

# Customize the plot
plt.xticks([0, 1], ['Non-Imp', 'Imp'])
plt.xlabel('Edge Importance')
plt.ylabel('||Q_e||_2')

# Show the plot
plt.savefig(f'Cont_edge_{noise_sd}.png')
plt.show()
